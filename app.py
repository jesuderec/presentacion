...
def generate_academic_content(texto_base, num_slides, model_name, api_key):
    ...
    # Configuramos lotes pequeños para que gpt-4o-mini no se agote
    lote_size = 5
    all_sections = []
    ...
    for i in range(0, num_slides, lote_size):
        ...
        st.write(f"⏳ Procesando bloque: Secciones {inicio} a {fin}...")

        prompt = f"""
        Actúa como Catedrático experto. Analiza este material: "{texto_base[:5000]}"
        Genera EXACTAMENTE {cantidad_lote} secciones de clase (de la número {inicio} a la {fin}).
        
        REQUISITOS POR SECCIÓN:
        1. Título académico específico.
        2. Mínimo 6 puntos clave (bullets) de revisión (máx 20 palabras cada uno).
        3. Narrativa argumentada de 350 palabras (guion del docente).
        4. 3 Sugerencias de imágenes (prompts) y 3 temas de video en español.
        ...
        {{
          ...
          "secciones": [
            {{
              "titulo": "...",
              "bullets": ["Punto 1", "Punto 2", "Punto 3", "Punto 4", "Punto 5", "Punto 6"],
              "guion_narrativo": "...",
              "imagenes_prompts": ["...", "...", "..."],
              "videos_youtube": ["...", "...", "..."]
            }}
...
def create_word_doc(data):
    ...
    for i, sec in enumerate(clase.get("secciones", [])):
        ...
        doc.add_heading("Puntos de Revisión (Contenido Visual)", level=2)
        for b in sec.get("bullets", []):
            doc.add_paragraph(b, style='List Bullet')
        
        doc.add_heading("Recursos Sugeridos (3 Imágenes | 3 Videos)", level=2)
        ...
        doc.add_heading("Narrativa y Argumentación", level=2)
...
def create_narrative_txt(data):
    ...
    for i, sec in enumerate(clase.get("secciones", [])):
        output += f"DIAPOSITIVA/SECCIÓN {i+1}: {sec.get('titulo')}\n" + "-"*30 + "\n"
...
st.title("🎓 Generador Académico Multi-Sección")

with st.sidebar:
    st.header("⚙️ Configuración")
    model = st.selectbox("Modelo:", ["gpt-4o-mini", "deepseek-chat"])
    num_slides = st.slider("Cantidad de secciones (hasta 25):", 3, 25, 15)
...
if st.button("🚀 Generar Todo"):
    ...
    if not contexto.strip():
        ...
        with st.spinner("Procesando por lotes para asegurar calidad..."):
...
if st.session_state.get('ok'):
    ...
    with c1: st.download_button("📥 Word Completo", s.getvalue(), "Plan_Clase.docx")
    with c2: st.download_button("📥 Narrativa Texto", st.session_state.txt, "Narrativa.txt")
